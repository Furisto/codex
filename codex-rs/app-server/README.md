...
- `environment/add` — experimental; add or replace a named remote environment in the built-in `static` provider by `environmentId` and `execServerUrl` for later selection by `thread/start` or `turn/start`; bare IDs are aliases for `static/<id>`, while qualified IDs must use the `static` provider; optional `connectTimeoutMs` overrides the WebSocket connection timeout; returns `{}` and does not change the default environment.
- `environment/info` — experimental; connect to a configured environment by canonical `providerId/environmentId` or a legacy bare static ID and return its detected `shell` plus its default `cwd` as a canonical environment-native `file:` URI. Connection failures are returned as request errors.
- `environmentProvider/create` — experimental; persist a dynamic provider with `name`, `kind`, optional `url`, and tagged PAT authentication. `kind: "static"` is rejected. Ona defaults to `https://app.gitpod.io/api`. The response contains the complete provider with authentication redacted to `{ "type": "pat" }`; creation validates syntax but does not contact the provider.
- `environmentProvider/update` — experimental; update only a dynamic provider's optional `name` and/or tagged PAT `authentication` by `providerId`. IDs, kinds, and URLs are immutable, and the response contains the complete redacted provider.
- `environmentProvider/list` — experimental; list the fixed `static` provider first, followed by dynamic providers ordered case-insensitively by name and ID. Accepts optional opaque `cursor` and `limit`; returns `{ data, nextCursor }`. Static remains available when dynamic provider storage is unavailable.
- `environmentProvider/delete` — experimental; delete a dynamic provider by `providerId`. Without `force`, deletion fails closed unless the provider can be queried and has no Codex-owned environments. With `force: true`, the server best-effort deletes discovered environments, removes the definition even when cleanup is incomplete, and returns `cleanup.status` as `complete`, `partial`, or `unknown` plus known `failedEnvironmentIds`. The built-in `static` provider cannot be deleted.
- `environment/create` — experimental; start asynchronous provisioning through a dynamic `providerId` using a required `{ repositoryUrl, ref }` source and opaque `resourceClass`. Returns only `{ environment: { providerId, environmentId } }`; lifecycle state is read separately and delivered through notifications. The `static` provider is rejected.
- `environment/read` — experimental; read one authoritative environment using `providerId` and its provider-native `environmentId`. Dynamic records contain source, resource class, and status; static records expose `source: null`, `resourceClass: null`, and `status.phase: "running"`.
- `environment/list` — experimental; list environments from one required `providerId`, with optional opaque `cursor` and `limit`, returning `{ data, nextCursor }`. Dynamic pagination belongs to the provider; static pagination uses a stable lexical keyset over currently registered environments.
- `environment/delete` — experimental; request asynchronous deletion using `providerId` and provider-native `environmentId`. An empty success response means the provider accepted the request; eventual completion is reported by `environment/deleted`. The `static` provider is rejected.

Lifecycle notifications are experimental: `environment/created` and `environment/updated` contain the complete environment record, while `environment/deleted` contains the canonical qualified `environmentId` (`providerId/environmentId`).
